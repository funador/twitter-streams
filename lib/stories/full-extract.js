...
module.exports = async (tweet, topic, id, url, retweeter) => {
  ...
  // Check if there is an article and image to work with2
  if (fluffed && fluffed.image) {
    ...
    const story = {
      ...
      descCheck, retweeters, timestamp, url, topic, id, count, image
    }
    ...
    if (Object.keys(story).every(key => story[key])) {
      ...
      const remove = ['matchingCheck', 'langCheck', 'ignoredCheck', 'descCheck']
