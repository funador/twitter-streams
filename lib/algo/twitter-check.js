      ...
      const twitter = snap.val()
        ...
        if (false) { 
          ...
          T.get('statuses/user_timeline', query, async (err, data) => {
            ...
            const diffLangs = data.filter(tweet => {
              return tweet.user.lang != tweet.lang
            }).length

            const likeRatio = likes / original.length || 0
            const retweetsRatio = retweets / original.length || 0
            const convoRatio = convos / original.length || 0

            const ranker = {
              mins, quotedStatus, withoutLinks, freqReweetedLen, freqLinksLen, 
              totalRetFreq, totalLinkFreq, totalTextFreq, chainedRetweetsLen, 
              likeRatio, retweetsRatio, convoRatio, diffLangs
            }
