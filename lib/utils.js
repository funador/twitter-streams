...
exports.yearCheck = user => {
  const momentTime = this.dateFomatter(user.created_at).fromNow().split(' ')
  
  if(momentTime[1].includes('years')) {
    return Number(momentTime[0])
  }

  if(momentTime[1].includes('year')) {
    return 1
  }
...
exports.topicsBuilder = (kloutObj, topics) => {
  return topics.reduce((obj, topic) => {
    topic = topic.replace(/[/\|.|$|#]/g, '')
    
    obj[topic] ? obj[topic]++ : obj[topic] = 1
    return obj
