...
const setup = async () => {
  ...
  //   .then(snap => {
  //     const obj = snap.val()
  //     const sorted = Object.keys(obj).sort((a, b) => obj[a].retweets - obj[b].retweets)
  //     console.log("DATA", sorted[sorted.length - 1])
  //   })
  ...
  const reducer = arr => arr.reduce((total, twitter) => {
    return total += twitter.diffLangs
  }, 0) / arr.length
...
// chainedRetweetsLen not working