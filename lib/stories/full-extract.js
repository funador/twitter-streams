...
const { firebase, ingornedLanguages, resizeHero, resizeThumb, cloudinary } = require('../config')
...
module.exports = async (tweet, topic, id, url, retweeter) => {
  ...
  // Check if there is an article to work with2
  if (fluffed) {
    ...
    if (Object.keys(story).every(key => story[key])) {
      ...
      // Save a reference so we can update the story if needed
      firebase
        .ref(trackingRef)
