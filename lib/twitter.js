...
import { stream, db, topics, ignoredDomains, langs } from './config'
...
function* initExtractGen(tweet) {
  ...
  const topic = topics.filter((topic) =>
                        textToCheck.toLowerCase().includes(topic))[0]
  ...
  db.ref(`tracking/${topic}/${id}`).transaction(story => {
      ...
      if(story && story.tracking) {
        ...
        db.ref(retweetersRef).once('value').then(snap => {
          ...
          // If it is not in the array update the retweeters array with the new
          // retweeter
...
function* fullExtractGen(tweet, topic, id, url, retweeter) {
  ...
  if(img && img.includes("http")) {
    ...
    const miner = Math.ceil(unfluffed.text.split(' ').length / 250)

    const read_mins = unfluffed.text ? miner : false
    ...
    const lang_check = langs.every(lang => unfluffed.lang != lang)
...
topics.forEach(topic => {
  ...
  // Make a listener for each topic and look at the oldest story for this topic
  db.ref(`${topic}`).orderByChild('timestamp')
                    .endAt(cutoff)
                    .limitToLast(1)
                    .on('child_added', snap => {
