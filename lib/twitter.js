...
import { stream, firebase, topics, ignoredDomains, langs } from './config'
...
function* initExtractGen(tweet) {
  ...
  const makeId = url.match(/[^_\W]+/g)
                    .join('')
                    .replace(/w/g, '')
                    .substring(5, 40)

  const id = url ? makeId : false
  ...
  firebase
    ...
    .transaction(story => {
      ...
      if(story && story.tracking) {
        ...
        firebase
          .ref(retweetersRef).once('value').then(snap => {
            ...
            firebase
              .ref(retweetersRef).set(retweeters)
        })
        return story
      }
