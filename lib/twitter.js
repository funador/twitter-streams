      ...
      if(story && story.tracking) {
        ...
        firebase
          ...
          .then(snap => {
            ...
            let story, { retweeters, count } = snap.val()
            ...
            if(retweeters.every(tweeter => tweeter.name != retweeter.name)) {
              story.retweeters = retweeters.push(retweeter)
              story.count = retweeters.length
            }
...
function* fullExtractGen(tweet, topic, id, url, retweeter) {
  ...
  const fluffed = yield request(url, (err, res, body) =>
  ...
  if(img && img.includes("http")) {
    ...
    const story = {
      ...
      desc_check, img, retweeters, timestamp, url, topic, id
    }
