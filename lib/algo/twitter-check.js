    ...
    try {
      ...
      if (twitter) { 
        ...
        T.get('statuses/user_timeline', query, async (err, data) => {
          ...
          const userObj = {
            full_name: singleTweet.user.name,
            name: singleTweet.user.screen_name,
          ...
          if(flag == 'stream') {
            let spamOrHam = twitterAlgo(sorted)
