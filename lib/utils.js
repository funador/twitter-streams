...
exports.ageCheck = tweet => {
  /// argh... twitter timestamp :/
  const busted = tweet.user.created_at.split(' ')
  const month = moment().month(busted[1]).format("MM")
  const dated = moment([year, busted[5], busted[2]].join(''), "YYYYMMDD").fromNow()
  return dated.includes('year') ? true : false
}
